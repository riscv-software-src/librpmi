...
init_attrs_expected(struct rpmi_test_scenario *scene,
...
static int verify_unknown_guid(struct rpmi_test_scenario *scene,
			       struct rpmi_test *test,
			       struct rpmi_message *msg)
{
	if (test_mm_callback_count) {
		printf("%s: unexpected MM callback count %d\n",
		       test->name, test_mm_callback_count);
		return 1;
	}
...
static int test_mm_scenario_init(struct rpmi_test_scenario *scene)
{
	...
	test_mm_shmem = rpmi_shmem_create("test_mm_shmem",
					  ...
					  sizeof(test_mm_mem), &rpmi_shmem_simple_ops,
					  NULL);
	if (!test_mm_shmem) {
